...
ROSE <- function(response_var = NULL, predictor_vars = NULL, data = NULL, Formula = NULL, N, p = 0.5, hmult.majo = 1, hmult.mino = 1, subset = options("subset")$subset, na.action = options("na.action")$na.action, seed) {
  ...
  if (!is.null(Formula)) {
    ...
    obj <- omnibus.balancing(response =  response_var, predictors =  predictor_vars, data =  data, subset =  subset, na.action =  na.action, N =  N, p =  p,  method = "rose", seed = seed, hmult.majo = hmult.majo, hmult.mino = hmult.mino)
  } else {
