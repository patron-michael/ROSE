...
ROSE.eval <- function(response = NULL, predictors = NULL, data = NULL, learner = NULL, formula = NULL,
                      acc.measure = "auc", extr.pred = NULL, method.assess = "holdout", K = 1, B = 100,
                      control.rose = list(), control.learner = list(), control.predict = list(),
                      control.accuracy = list(), trace = FALSE, subset = options("subset")$subset,
                      na_action = options("na.action")$na.action, seed = NULL) {
  
  # Check if both formula and predictors are provided
  if (!is.null(formula) && !is.null(predictors)) {
    stop("Both 'formula' and 'predictors' cannot be provided simultaneously. Please choose one.\n")
  }
  
  # Check if either formula or predictors is provided
  if (is.null(formula) && is.null(predictors)) {
    stop("Either 'formula' or 'predictors' must be provided.\n")
  }
  
  # If formula is provided, extract response and predictors
  if (!is.null(formula)) {
